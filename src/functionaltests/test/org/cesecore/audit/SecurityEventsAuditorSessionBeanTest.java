...
public class SecurityEventsAuditorSessionBeanTest extends SecurityEventsBase {
    ...
    public void test02SelectLogsWithCriteria() throws AuthorizationDeniedException{
    	for (final String logDeviceId : securityEventsAuditor.getQuerySupportingLogDevices()) {
    		final List<? extends AuditLogEntry> list = securityEventsAuditor.selectAuditLogs(roleMgmgToken, 1, 10, QueryCriteria.create().add(Criteria.eq(AuditLogEntry.FIELD_AUTHENTICATION_TOKEN, "userid")), logDeviceId);
    ...
    public void test05ExportLogs() throws Exception {
    	...
    	for (final String logDeviceId : securityEventsAuditor.getQuerySupportingLogDevices()) {
            final String file0 = securityEventsAuditor.exportAuditLogs(roleMgmgToken, cryptoToken, new Date(), false, keyAlias, keyPairSignAlgorithm, logDeviceId).getExportedFile();
            ...
            final String file1 = securityEventsAuditor.exportAuditLogs(roleMgmgToken, cryptoToken, new Date(), true, keyAlias, keyPairSignAlgorithm, logDeviceId).getExportedFile();
            ...
            final String file2 = securityEventsAuditor.exportAuditLogs(roleMgmgToken, cryptoToken, new Date(), true, keyAlias, keyPairSignAlgorithm, logDeviceId).getExportedFile();
            ...
            // This test only applies to the QueuedDevice..
            if (logDeviceId.equals("QueuedDevice")) {
                // If there is no configuration the audit logs are not deleted. When they are deleted the last signed log is left.
                final LogManagementData currentConfiguration = logManagement.getCurrentConfiguration(roleMgmgToken);
                if (currentConfiguration == null) {
                    assertTrue(String.format("f2 length %d is not > f1 length %d", length2, length1), length2 > length1);
                } else {
                    assertTrue(String.format("f2 length %d is not < f1 length %d", length2, length1), length2 < length1);
                }
            }
    ...
    public void test06Authorization() throws Exception {
        final X509Certificate certificate = CertTools.genSelfCert("C=SE,O=Test,CN=Test LogMgmtSessionNoAuth", 365, null, keys.getPrivate(), keys.getPublic(),
    	...
    	for (final String logDeviceId : securityEventsAuditor.getQuerySupportingLogDevices()) {
            ...
            try {
                securityEventsAuditor.selectAuditLogs(adminTokenNoAuth, 1, 10, QueryCriteria.create().add(Criteria.orderDesc(AuditLogEntry.FIELD_TIMESTAMP)), logDeviceId);
